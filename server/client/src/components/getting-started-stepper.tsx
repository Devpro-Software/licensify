...
export default function GettingStartedStepper(props: Props) {
    ...
    const { signature } = useSignature(props.license.id)

    const steps: ReactNode[] = [
        <div>
            {signature && <SignatureBuilder mutatePath={`/api/licenses/${props.license.id}/trackers`} licenseId={props.license.id} title="Build a license signature" description="Store this near your client code and use it for validation. Add an optional tracker which has server side features." sig={signature} />}
        </div>,
        <div>
            <GettingStartedCode />
        </div>,
