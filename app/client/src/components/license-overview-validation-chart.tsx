...
export function LicenseOverviewValidationBarChart() {
    const [showing, setShowing] = useState(defaultShowingCount)

    const router = useRouter()
    const { data, isLoading } = useSWR("/api/validations/activity?type=licenses")
        ...
        <Card>
            ...
            <CardFooter className="flex-col items-start gap-2 text-sm">
                ...
                <div className="leading-none text-muted-foreground">
                    Showing your top {cdata.length} licenses
                </div>
            </CardFooter>
        </Card>
    )
